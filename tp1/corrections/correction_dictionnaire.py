...
for m in texte_en_mots:
    # On vérifie d'abord si le mot est déjà une clé
    # du dictionnaire: si oui, on l'utilise, sinon on la crée.
    if m in mots:
        mots[m] += 1
    else:
        # On crée la nouvelle clé et on initialise la valeur
        mots[m] = 1
...
for c in texte:
    # On vérifie d'abord si le caractère est déjà une clé
    # du dictionnaire: si oui, on l'utilise, sinon on la crée.
    if c in lettres:
        lettres[c] += 1
    else:
